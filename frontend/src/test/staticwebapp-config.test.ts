...
describe('CSP policies', () => {
  ...
  it('prod CSP connect-src pins a specific API origin (no wildcard backends)', () => {
    const connectSrc = (prodCsp as Record<string, string[]>)['connect-src']
    // The prod CSP must allow the backend API via an explicit origin rather
    // than a wildcard — assert at least one non-Clerk/non-Stripe/non-Sentry
    // https origin is present (set this to your API domain in csp.prod.json).
    const apiOrigins = connectSrc.filter(
      (v) =>
        v.startsWith('https://') &&
        !v.includes('*') &&
        !/clerk|stripe|sentry|telemetry/.test(v),
    )
