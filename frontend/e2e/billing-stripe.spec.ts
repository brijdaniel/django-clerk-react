...
test.describe('Stripe Billing Integration', () => {
  ...
  test('subscribe via PricingTable with Stripe test card', async ({ page }) => {
    ...
    await expect(page.getByText('Prepaid balance').first()).toBeVisible({ timeout: 10000 })
  ...
  test('seed usage and generate invoice', async ({ page }) => {
    ...
    // Dynamically compute backdate so it always lands ~15th of previous month.
    const backdate = daysIntoPreviousMonth()
    await seedUsage(page, { usage_type: 'api_call', amount: '2.50', description: 'E2E: 50 api calls', backdate_days: backdate })
    await seedUsage(page, { usage_type: 'report', amount: '1.00', description: 'E2E: 5 reports', backdate_days: backdate })
