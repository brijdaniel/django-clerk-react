#!/bin/bash
# Manage the app's Azure infrastructure (non-deployment operations).
...
while IFS='=' read -r key value; do
  ...
  case "$key" in
    ...
    APP_NAME)             APP_NAME="$value"; PARAMS="$PARAMS $key=$value" ;;
...
# Container app names — must match how main.bicep derives them from APP_NAME.
APP_NAME="${APP_NAME:-app}"
API_APP="${APP_NAME}-api-${ENVIRONMENT_NAME}"
WORKER_APP="${APP_NAME}-worker-${ENVIRONMENT_NAME}"
BEAT_APP="${APP_NAME}-beat-${ENVIRONMENT_NAME}"
